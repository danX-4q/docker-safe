...
export INCLUDE_SOFS="true"

##############################

function __so__parm_to_obj__chainpos()
{
    local block_num=$1
    local tx_index=$2
...
function __so__parm_to_obj__tx()
{
    local account=$1
    local txid=$2
    local outidx=$3
...
function __so__parm_to_obj__txkey()
{
    local txid=$1
    local outidx=$2
...
function so__push0cctx()
{
    ...
    local cpos_obj=$(__so__parm_to_obj__chainpos $cpos)  #do not use "$x"
    local npos_obj=$(__so__parm_to_obj__chainpos $npos)  #do not use "$x"
...
function so__push1cctx()
{
    ...
    local cpos_obj=$(__so__parm_to_obj__chainpos $cpos) #do not use "$x"
    local npos_obj=$(__so__parm_to_obj__chainpos $npos) #do not use "$x"
    local tx1_obj=$(__so__parm_to_obj__tx $tx1)         #do not use "$x"
...
function so__push2cctx()
{
    ...
    local npos_obj=$(__so__parm_to_obj__chainpos $npos) #do not use "$x"
    local tx1_obj=$(__so__parm_to_obj__tx $tx1)         #do not use "$x"
    local tx2_obj=$(__so__parm_to_obj__tx $tx2)         #do not use "$x"
...
function so__draw1asset()
{
    local txk1="$1"
    local account="$2"
    local txk1_obj=$(__so__parm_to_obj__txkey $txk1)    #do not use "$x"
...
function so__draw2asset()
{
    ...
    local account="$3"
    local txk1_obj=$(__so__parm_to_obj__txkey $txk1)    #do not use "$x"
    local txk2_obj=$(__so__parm_to_obj__txkey $txk2)    #do not use "$x"
